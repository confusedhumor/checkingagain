...
//   // const [rows] = await connection.query(sql, [userInfo.phone, userInfo.code]);
...
const listGames = async (req, res) => {
  ...
  // Define the SQL query to fetch data from all three tables
  const sql = `
    (SELECT id_product, phone, code, invite, stage, result, level, money, amount, fee, get, game, bet, status, time 
     FROM minutes_1 
     WHERE phone = ? AND code = ?)
    UNION ALL
    (SELECT id_product, phone, code, invite, stage, result, level, money, amount, fee, get, game, bet, status, time 
  ...
  // Execute the query to fetch data
  const [rows] = await connection.query(sql, [userInfo.phone, userInfo.code, userInfo.phone, userInfo.code, userInfo.phone, userInfo.code]);
